 ...
 */
function evalInstr(instr: bril.Instruction, state: State, numactions:boolean): Action {
  //console.log(JSON.stringify(instr))
  if (numactions) {
    Deno.writeTextFileSync("getpath.json", "," + JSON.stringify(instr), { append: true });
  } else {
    Deno.writeTextFileSync("getpath.json", JSON.stringify(instr), { append: true });
  }
  ...
  case "br": {
    ...
    Deno.writeTextFileSync("getpath.json", "," + JSON.stringify({"branch value": cond}), { append: true });
    if (cond) {
      return {"action": "jump", "label": getLabel(instr, 0)};
    } else {
  ...
  case "call": 
    Deno.writeTextFileSync("getpath.json", ",", { append: true });{
...
function evalFunc(func: bril.Function, state: State): Value | null {
  let numactions = false;
  for (let i = 0; i < func.instrs.length; ++i) {
    let line = func.instrs[i];
    if ('op' in line) {
      // Run an instruction.
      let action = evalInstr(line, state, numactions);
      numactions = true;
...
async function main() {
  Deno.writeTextFileSync("getpath.json", "{\"path\":[");
  try {
